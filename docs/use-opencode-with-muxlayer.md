...
中文：[让 OpenCode 通过 MuxLayer 切换多供应商模型](./use-opencode-with-muxlayer-zh.md)
...
- The `muxlayer` virtual model (`agentgate` still works) to resolve to the currently selected route model.
...
| `openai/muxlayer` virtual model (`openai/agentgate` still works) | Route-selected real model | Provider-specific model ID |
...
| You changed providers but OpenCode still sends an old model name | Use the `muxlayer` virtual model path rather than hardcoding a provider model. |

## Related

- [Use Gemini CLI with MuxLayer](./use-gemini-cli-with-muxlayer.md)
